...
> ![](./camemberts.png){width=70%} [source de l'image](https://www.data-to-viz.com/caveat/pie.html)
>
>
...
![](./plot_axis_1.png){width=70%} ![](./plot_axis_2.png){width=70%}
...
De manière générale, évitez de faire des graphiques trop compliqués avec des fioritures. Evitez la 3D, les axes doubles et les lignes de grille. Concentrez vous sur l'essentiel! 
...
Voici un exemple d'un graphique avec des catégories non ordonnées, puis avec des catégories ordonnées. La version avec les catégories ordonnées est beaucoup plus facile à interprêter.
...
#### 2. Evitez de mettre trop d'information dans vos graphiques

Il est tentant de vouloir faire rentrer toutes nvs données dans un même graphique, surtout qu'avec R vous pouvez jouer avec les couleurs, les formes et la taille des points. Mais pour que le message ressorte bien, il vaut mieux présenter moins de données et se concentrer sur **un message par graphique**. 
...
![](./spaghetti.png){width=70%}

Pour éviter ce problème, vous pouvez faire un graphique par ligne, ou mettre en valeur une ligne en particuler.
...
Il existe d'ailleurs la librarie `RColorBrewer` qui contient de nombreuses palettes prédéfinies qui vous aideront beaucoup. 
...
Surtout vous pouvez facilement y selectionner les palettes de couleurs qui sont aussi visible pour les daltoniens avec la commande:
