...
- Quand vous ecrivez un chemin de dossiers dans R, utilisez des guillemets `"`

- Utilisez la touche `Tab` quand vous écrivez le chemin du dossier, - _RStudio_ complètera le chemin pour vous 


- Le chemin peut être _relatif_ à votre répertoir de travail actuel. Pour cela utilisez le symbole e moment
    - Utilisez le symbole `./` pour indiquer votre répertoire de travail actuel
    - Utilisez le symbole `../` pour indiquer le répértoire _avant_ votre répertoire de travail actuel



## Importer des données dans R

Afin de pouvoir importer des données dans R, il faut connaitre leur format. S'agit-il de données `.txt` avec les colonnes séparées par _tabulation_ ou de données `.csv` avec les colonnes séparées par des _virgules_ ou des _point-virgules_? Il est important de connaitre la réponse à cette question car nous allons utiliser des fonctions différentes pour lire chaque type de données
...
_Note:_ Il existe aussi une fonction `read.csv()`, qui est la fonction par défault pour importer des données csv, et qui est très similaire à `read_csv()`. Pour ce cours, nous allons utiliser les fonctions `read_tsv()` ou  `read_csv()` du package `tidyverse` car elles sont plus pratiques.
...
- Installez R et Rstudio sur votre orginateur ([Installer R](https://cran.biotools.fr/), [Installer Rstudio](https://rstudio.com/products/rstudio/download/))

- Organiser un de vos tableur excel avec les Les colonnes qui contiennent  les variables (ce qui est mesuré) et les lignes qui contiennent les observations (les unités de l'étude). Sauvez le en format `.txt` avec les colonnes séparées par _tabulation_
