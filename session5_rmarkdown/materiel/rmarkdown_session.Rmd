...
Nous allons donc illustrer les possibilités offertes par ce langage en faisant une courte analyse en R des données de Burghardt et Al. (2015), et en exportant ces analyses dans un rapport complet et documenté.
...
La syntaxe complète de Rmarkdown, dont vous allez avoir besoin par la suite, est dans cette [antisèche](https://raw.githubusercontent.com/rstudio/cheatsheets/master/rmarkdown-2.0.pdf), que vous pouvez garder ouverte au cours du TP.
...
Compilez le document. Vous devirez remarquer 
...
Les couleurs gloables du document (liens, couleur de fond, table des matières, etc) peuvent être modifiées. Ceci aura de nouveau lieu dans l'entête YAML de votre document, en utilisant les options `theme` et `highlight`. 
...
Changez le `theme` du document dans l'entête YAML.
