...
Le premier chunk doit être conservé. Sa fonction `knitr::opts_chunk$set` permet de configurer des options communes aux chunks du documents. En l'occurrence, par défaut, cette fonction impose ```echo = TRUE``` pour tous les chunks du document.
...
> **Attention** N'essayez pas dans la suite du TP de changer le répertoire du travail avec `setwd()`, car dans les documents Rmarkdown, le répertoire de travail est toujours le répoertoire dans lequel le document Rmarkdown est enregistré. A l'avenir, nous nous déplacerons toujours de manière relative à ce répertoire là.
...
Nous souhaitons maintenant montrer les données brutes au lecteur du rapport. Dans le chunk d'import des données, vous pouvez appliquer les fonctions que vous souhaitez pour inspecter les données, comme vu dans les séances précédantes.
...
Compilez. Vous devriez voir les données (en partie), mais elles ne sont pas présentées d'une manière très agréable à lire.
...
En dehors du chunk, décrivez la visualisation que vous souhaitez faire. Ici, nous allons montrer le nombre de feuilles des rosettes (variable `rosette.leaf.num`) en fonction du nombre de jours jusqu'à floraison (variable `days.to.flower`), et séparer les observations par une la couleur correspondant à la photopériode (variable `day.length`). Réalisez ce graphe avec `ggplot2` (n'oubliez pas de lui donner un titre) dans un nouveau chunk.
...
+ Ajoutez dans l'entête YAML un champs `bibliography`
...
Compilez le document. Vous devriez remarquer qu'à l'endroit de la citation dans le contexte, le nom et la date de la publication apparaissent entre parenthèse. De plus, la référence détaillée a dû apparaître en bas du rapport.
...
A partir de ce que nous avons vu aujourd'hui, choisissez le script `.R` d'une session précédante, comme par exemple la séance de découverte de `ggplot`, et créez un rapport en `Rmarkdown` à partir de ce sript. Vous pouvez par exemple créer un chunk par question ou notion principale de la séance, rdécrire ce que fait chaque chunk au dessus ou en dessous, et créer une architecture (sections, sous-section) pertinente au rapport.
