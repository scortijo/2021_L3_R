...
Avec la syntaxte tidyverse, on commence par grouper les observations suivant le `cluster` avec la fonction `group_by` appliquée à `expt1`.  On chaine la sortie de regroupement avec `summarise`, en demandant une colonne nommée `n.obs` qui sera le comptage du nombre d'observations dans chaque cluster. Cette fonction de comptage est `n()`.
...
**En conclusion, presque tous les gènes qui répondent à la chaleur sont dans le cluster 6A (sauf 1) : ce cluster est clairement enrichi en gènes de réponse à la chaleur.**
