...
Ouvrez avec `Rstudio` le fichier `Script_ggplot_lesson.R` qui est dans le répertoire `ggplot_session/materiel`.
...
Ces packages sont normalement déjà installé. Pour les importer dans votre session, utilisez la fonction ``library()`` : 
...
``` {r}
setwd("~/Desktop/2021_L3_R/ggplot_session/materiel")
```


# Import et vérification des données

Ouvrez avec `Rstudio` le fichier `Script_ggplot_session.R` qui est dans le répertoire `ggplot_session/materiel`. Nous allons coder ensemble dans ce script, qui est quasiment vide pour le moment. 


La version simplifiée des données est dans le dossier ``ggplot_session/data`` (`burghardt_et_al_2015_expt1.txt`). Il s'agit de données pour des phénotypes associés au temps nécessaire à des plantes de différents génotypes pour fleurir dans différentes conditions.
