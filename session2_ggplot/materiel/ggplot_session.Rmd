...
Pour cette formation, nous allons utiliser une version légèrement modifiée du jeu de donnée publié par [Burghard et al 2015](http://datadryad.org/resource/doi:10.5061/dryad.65d76). 
...
> **Challenge** Quel est le type de données le plus courant dans le jeu de données ? 
...
>**Exercice:** Nous voulons étudier la relation entre le nombre de feuille de rosette et la longueur la limbe des feuilles (en mm) pour les génotypes. 
>
...
>**Exercice:** Les couleurs n'apportent plus d'information supplémentaire. Pensez à une utilisation plus intéressante des couleurs dans ce graphique. 
...
>**Exercice:** Modifiez le graphique précédent afin de séparer les données (facet) en fonction de `fluctuation` en ligne, et `day.length` en colonne et de colorer les points en fonction du génotype.
...
- Si vous n'avez jamais utilisé `tidyverse` sur R, vous aurez besoin de l'installer avant de le charger dans R. Pour cela utilisez la commande suivante:
