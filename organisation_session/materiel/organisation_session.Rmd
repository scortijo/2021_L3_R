...
Il ets crucial d'avoir des données bien organisées afin de pouvoir les étudier correctement. Bien que formatter, nettoyer et organiser les données peut être long et pénible, c'est une étape essentielle pour l'analyse utérieure des données.

La plupart d'entre nous utilisent à un moment donné ou à un autre, un tableur ("Excel") pour y enregistrer des données. Nous allons voir dans excel:

- comment évter les pièges courants quand nous enregistrons des données

- les bonnes habitudes pour organiser les données

- les diffétents formats et leurs compatibilité avec d'autres programmes (comme ``R``)
...
## Bonnes habitudes pour organiser les données
