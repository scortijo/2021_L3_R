...
``` {r}
setwd("~/Desktop/2021_L3_R/ggplot_lesson/materiel")
```


# Import et vérification des données

Ouvrez avec `Rstudio` le fichier `Script_ggplot_lesson.R` qui est dans le répertoire `ggplot_lesson/materiel`. Nous allons coder ensemble dans ce script, qui est quasiment vide pour le moment. 


La version simplifiée des données est dans le dossier ``ggplot_lesson/data`` (`burghardt_et_al_2015_expt1.txt`). Il s'agit de données pour des phénotypes associés au temps nécessaire à des plantes de différents génotypes pour fleurir dans différentes conditions.
